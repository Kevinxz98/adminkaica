...
import { ChartAgent } from '../../../../components/agents/chart-agent/chart-agent';
...
import { ChatbotConversations } from '../../../../components/agents/chatbot-conversations/chatbot-conversations';
...
export class ViewAgent implements OnInit{
  ...
  ngOnInit(): void {
    ...
    this.clientsService.getBasicStats(publicKey).subscribe((data) => {
      this.isLoading = true;
  ...
  loadSevenDaysStats(publicKey: string): void {
    this.clientsService.getSevenDaysStats(publicKey).subscribe({
      next: (data) => {
        this.chatbotStats = data;
        this.isLoading = false;
        this.chatbotId = this.chatbotStats.chatbot_info.id;
